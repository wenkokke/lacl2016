...
\[
  ...
  \end{scprooftree*}
\]
It should be noted that the $\uparrow\downarrow$-rule can serve as a
normal-form for the \I\B\C-rules. Briefly, this is true because the
\I\B\C-rules can do nothing but move a constituent up, or down along
an existing path---the $\uparrow\downarrow$-rule mere captures this
more succintly.
...
Since proof search with the $\uparrow\downarrow$-rule eliminates the
need for the \I-rule, it is decidable.
To see why, note that the $\uparrow\downarrow$-rule introduces a
hollow product, and cannot move a constituent upwards past such a
product.
Therefore, an infinite loop in the search caused by this rule will
invariable contain a repeating sequent.
However, this will not stop the search from attempting to raise and
lower every sequence.
Worse: the system is capable of raising a second constituent while a
first has yet to come down. This means that for a sequent with $n$
(possibly nested) structures, we have $n!$ different ways to raise
all of these---and after every application of a logical rule, we can
attempt to raise and lower each of these again.\footnote{%
  We can restrict the search by introducing a quantifier
  license---e.g.\ a unary connective \textbf{Q}---and eliminating this
  license when \emph{raising}.
  In the same vein, we can use a quantifier license to obtain
  decidable proof search for our axiomatic system, by restricting
  access to the problematic $\I$-rule by requiring a \textbf{Q}.
}

To solve this, we can derive two rules, corresponding to
\citepos{barker2015} $\himpl{L_{\lambda}}$- and
$\himpr{R_{\lambda}}$-rules.
...
\[
  ...
  \end{scprooftree*}
\]
As these rules eliminate a logical connective, they enjoy the
subformula property.
Therefore, proof search with these rules is decidable.
Even better, it is \emph{efficient}, as the $qL$ and $qR$ rules can
only apply to quantifiers.

In fact, proof search with $qL$ and $qR$ is complete.\footnote{%
  In addition to the proof sketch below, this can be proven using a
  variant of \citepos[][ch.\ 17.6 and 17.7]{barker2015} proof of
  equivalence between {\NLLAM} and {\NLCL}.
}
Briefly, let us assume that the $\himpr,\himpl$-connectives only occur
in syntactic categories as part of the larger quantifier construction
${C}\himpl({A}\himpr{B})$.
If this is the case, then we still have access to all the same
operations that the $\uparrow\downarrow$-rule provided.
Now let us consider what other constructions the
$\himpr,\himpl$-connectives can meaningfully occur in.
By our choice of {\I}, {\B} and {\C} rules, we always raise the
quantifier to the top-left position, joined to a hollow product.
There are two interesting cases: either
\begin{enumerate*}[label= (\arabic*)]
\item%
  the main connective of the quantifier is $\himpl$, so that it could
  take the right-hand side of the hollow product as an argument; or
\item
  the main connective on the right-hand side of the hollow product is
  $\himpr$, so that it could take the quantifier as an argument.
\end{enumerate*}
In the first case, we can simply apply {$\himpl L$}.
The second case, however, cannot occur, as we know from the definition
of $\trace{\cdot}$ that the main connective on the right-hand side
must be a solid product.\footnote{%
  Note that this property is not necessarily preserved if we extend
  the logic with other structural rules mentioning the hollow
  connectives.
}
Therefore, the only case is for the main connective in the quantifier
to be $\himpl$. By a similar argument, we can show that the second
connective in the quantifier must, in fact, be $\himpr$, leading to
the ${C}\himpl({A}\himpr{B})$ structure.
...
translation to fNL, and then extend it to cover {f\NLCL}.
...
examples (\ref{ex:scope-ambiguity}-\ref{ex:changing-result-type}).
...
analysis. The remaining example, (\ref{ex:changing-result-type}), is
our improvement over their work, and motivated us to start from
